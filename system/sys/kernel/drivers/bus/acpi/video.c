...
acpi_video_bus_DOS(struct acpi_video_bus *video, int bios_flag, int lcd_flag)
{
	...
	status = acpi_evaluate_object(video->handle, "_DOS", &args, NULL);
      Failed:
	return_VALUE(status);
}
...
static void acpi_video_device_find_cap(struct acpi_video_device *device)
{
	...
	if (obj && obj->type == ACPI_TYPE_PACKAGE && obj->package.count >= 2) {
		...
		br = kmalloc(sizeof(*br), MEMF_KERNEL);
		if (!br) {
			printk("can't allocate memory\n");
		} else {
			memset(br, 0, sizeof(*br));
			br->levels = kmalloc(obj->package.count *
					     sizeof *(br->levels), MEMF_KERNEL);
			if (!br->levels)
				goto out;
			...
			if (count < 2) {
				...
				printk( "ACPI: Found %d brightness levels\n",
						  count );
				//acpi_video_device_lcd_set_level( device, 20 );
			}
		}
	}
			      ...
			      struct acpi_video_bus *video)
{
	...
	if (ACPI_SUCCESS(status)) {
		...
		data = kmalloc(sizeof(struct acpi_video_device), MEMF_KERNEL);
		if (!data)
			return_VALUE(-ENOMEM);
...
//		printk( "Device ID %x\n", (uint)device_id );
		...
		case 0x0100:
			data->flags.crt = 1;
			printk("ACPI: Found CRT device\n");
			break;
		case 0x0400:
			data->flags.lcd = 1;
			printk("ACPI: Found LCD device \n");
			break;
		case 0x0200:
			data->flags.tvout = 1;
			printk("ACPI: Found TV device\n");
			break;
		default:
...
static int acpi_video_switch_output(struct acpi_video_bus *video, int event)
{
	...
	list_for_each_safe(node, next, &video->video_device_list) {
		...
		if (state & 0x2) {
			if( node->next == &video->video_device_list )
				dev_next = container_of(video->video_device_list.next, struct acpi_video_device,
					 entry);
			else
				dev_next =
...
static int acpi_video_bus_put_devices(struct acpi_video_bus *video)
{
	...
	list_for_each_safe(node, next, &video->video_device_list) {
		...
		if (ACPI_FAILURE(status))
			printk(			       "hhuuhhuu bug in acpi video driver.\n");
...
static void acpi_video_bus_notify(acpi_handle handle, u32 event, void *data)
{
	...
	case ACPI_VIDEO_NOTIFY_SWITCH:	/* User request that a switch occur,
		...
		printk("Switch event\n");
		acpi_video_switch_output(video, ACPI_VIDEO_NOTIFY_CYCLE );
		break;
...
static int acpi_video_bus_add(struct acpi_device *device)
{
	...
	video = kmalloc(sizeof(struct acpi_video_bus), MEMF_KERNEL);
	if (!video)
		return_VALUE(-ENOMEM);
	...
	printk("%s [%s] (multi-head: %s  rom: %s  post: %s)\n",
...
static void acpi_video_exit(void)
{
