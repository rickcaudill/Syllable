...
//	void * sysctl_table;
//    unsigned char ctr; // shadow control register
};
 ...
 * interface.
 **/
/* sand static __inline__ int parport_yield(struct pardevice *dev)
{
 ...
 **/
/* san static __inline__ int parport_yield_blocking(struct pardevice *dev)
{
	unsigned long int timeslip = (jiffies - dev->time);
	if ((dev->port->waithead == NULL) || (timeslip < dev->timeslice))
		return 0;
...
/* san static __inline__ void parport_generic_irq(int irq, struct parport *port,
					   struct pt_regs *regs)
{
